...
class MateHmsMixin(models.AbstractModel):
    ...
    def mate_prepare_invocie_data(self, partner, patient, product_data, inv_data):
        ...
        if Module.search([('name','=','mate_hms_commission'),('state','=','installed')]) and self.env.context.get('commission_partner_ids',False):
            data['commission_partner_ids'] = [(6, 0, [self.env.context.get('commission_partner_ids')])]
        return data

    @api.model
    def mate_create_invoice(self, partner, patient=False, product_data=[], inv_data={}):
        ...
        for data in product_data:
            ...
            if product:
                ...
                if not data.get('price_unit'):
                    price = product.with_context(mate_pricelist_id=mate_pricelist_id)._mate_get_partner_price(quantity, uom_id, partner)
                    discount = product.with_context(mate_pricelist_id=mate_pricelist_id)._mate_get_partner_price_discount(quantity, uom_id, partner)
                else:
    ...
    @api.model
    def mate_create_invoice_line(self, product_data, invoice):
        ...
        if product:
            ...
            if not product_data.get('price_unit'):
                price = product.with_context(mate_pricelist_id=mate_pricelist_id)._mate_get_partner_price(quantity, uom_id, invoice.partner_id)
                discount = product.with_context(mate_pricelist_id=mate_pricelist_id)._mate_get_partner_price_discount(quantity, uom_id, invoice.partner_id)
            else:
    ...
    def mate_action_view_invoice(self, invoices):
...
class MateDocumentMixin(models.AbstractModel):
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'attachment_mate_hms_rel', 'record_id', 'attachment_id', compute="_mate_attachemnt_count", string="Attachments")
