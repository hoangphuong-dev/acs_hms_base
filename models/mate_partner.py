...
class ResPartner(models.Model):
    ...
    mate_patient_id = fields.Many2one('mate_hms.patient', compute='_is_patient', string='Patient', readonly=True)
    ...
    def _is_patient(self):
        Patient = self.env['mate_hms.patient'].sudo()
        for rec in self:
    ...
    def _patient_search(self, operator, value):
        patients = self.env['mate_hms.patient'].sudo().search([])
        return [('id', 'in', patients.mapped('partner_id').ids)]

    def create_patient(self):
        self.ensure_one()
        patient_id = self.env['mate_hms.patient'].create({
            'partner_id': self.id,
            'name': self.name,
        })
        return patient_id
